use std::{ops::{Deref, DerefMut}, iter::Peekable};




pub struct SortedIter<T: Iterator>(Peekable<T>);

impl<T: Iterator> Deref for SortedIter<T> {
    type Target = Peekable<T>;

    fn deref(&self) -> &Self::Target { &self.0 }
}

impl<T: Iterator> DerefMut for SortedIter<T> {
    fn deref_mut(&mut self) -> &mut Self::Target { &mut self.0 }
}

impl<T: Iterator> SortedIter<T> {
    pub unsafe fn from_unchecked(iter: T) -> Self {
        Self(iter.peekable())
    }
    pub fn next_contains(&mut self, item: T::Item) -> bool where T::Item : Ord {

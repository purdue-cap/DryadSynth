...
pub fn encode_file(path: &str) -> Result<Vec<u8>> {
    // read the file and change to AST with sygus-parser API
    let text = fs::read_to_string(path)?;
    let ast = SyGuSFile::from_str(&text)
        .map_err(|e| anyhow::anyhow!("parse error: {e}"))?;

    // create a new capnp msg
    use crate::sygus_capnp::sygus_problem;
    let mut msg = Builder::new_default();
    {
        let mut root = msg.init_root::<sygus_problem::Builder>();

        // find 'set-logic' from AST
        ...
        // find 'synth-fun' from AST for name, var, return type, and grammar
        ...
        for (i, (name, params, ret_sort, grammar_opt)) in synths.iter().enumerate() {
            ...
            // Set grammar after parameter setting
            let gram_str = grammar_opt
                .as_ref()
                .map(|g| format!("{g:?}"))
                .unwrap_or_default();
            fd.set_grammar(gram_str.as_str().into());
        }

        // Constraint list
        let cons: Vec<_> = ast.cmds.iter().filter_map(|c| {
            if let SyGuSCmd::Constraint(expr) = c { Some(expr) } else { None }
        }).collect();

        let mut cs = root.reborrow().init_constraints(cons.len() as u32);
        for (i, e) in cons.iter().enumerate() {
            cs.reborrow().get(i as u32).set_expr(format!("{e:?}").as_str().into());
        }
    }
